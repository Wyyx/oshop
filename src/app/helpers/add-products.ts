...
export function loadProducts(http: AuthHttp) {
	const products = [
		{
			category: { id: 5, name: 'vegetables' },
			imageUrl:
		...
		{
			category: { id: 4, name: 'seasonings and spices' },
	...
	for (let p of products) {
		...
		http.post('/api/products', JSON.stringify(p)).subscribe()
	}
}
